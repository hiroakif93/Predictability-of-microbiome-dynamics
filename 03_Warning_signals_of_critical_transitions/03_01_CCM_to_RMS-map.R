...
dir <- make.dir('03_Warning_signals_of_critical_transitions/Stablity')
...
for(i in names(dlist)[-7]){ # i=names(dlist)[3]
	...
	for(r in 1:8){#r=1
		...
		## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##
		cat(sprintf(" Prediction replicate %s...", r))
		## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##
    ...
    for(surr in which(replicateres$best==1 & replicateres$delta.rmse<0 & 
        ...
        forir.data <- matrix(NA, ncol= srrogate.ittr, nrow=nrow(ts.scale))
        for(r in 1:8){#r=1
	...
	# result <- readRDS(sprintf('%s/CCM_crosvaridation_%s.rds', dir$rdsdir, gsub('/', '_', i)))
	cat("\n\nStart RMS-map\n"); rmstimeS <- Sys.time()

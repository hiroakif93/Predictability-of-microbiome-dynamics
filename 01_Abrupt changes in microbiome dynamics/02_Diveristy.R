...
dir <- make.dir('01_Abrupt changes in microbiome dynamics/Diversity')
...
for(i in names(dlist)[-7]){ 
    ...
    smlsub$shannon <- apply(ts.sub, 1, function(x) d(x, lev='alpha', q=1))
...
saveRDS(smlsum[,11:13], 'Table/01_Diversity.rds')
sml <- smlsum
############################################################################
...
pdf(sprintf('%s/Mean coefficient of variation.pdf', dir$figdir), w=6, h=6)
plot(cvg)
dev.off()
############################################################################
...
ggsave(plot=alpha,
       file=sprintf('%s/Alpha diveristy.pdf', dir$figdir),
       w=6.5, h=6)
...
ggsave(plot=g,
       file=sprintf('%s/Time series of Alpha diveristy.pdf', dir$figdir),
       width=10,heigh=13)
...
ggsave(plot=plot_grid(plotlist=gglist, byrow=FALSE),
       filename=sprintf('%s/Bray-Curtis dissimilarity of community structure between time points.pdf', dir$figdir),
       width=10,heigh=8)
...
ggsave(plot=g1, file=sprintf('%s/Bray-Curtis dissimilarity among replicate.pdf', dir$figdir), h=8, w=21) 
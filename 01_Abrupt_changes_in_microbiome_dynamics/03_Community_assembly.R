...
dir <- make.dir('01_Abrupt_changes_in_microbiome_dynamics/Community_assembly')
...
mag <- unique(readRDS('Table/ShotgunMetagenome/MAG_infomation.rds')) 

color <- readRDS('Table/color_palette.rds')

## -- ASV X_0003 showed same dynamics with MAG
taxa['X_0003', 2:8] <- as.character(as.matrix(mag)[22,4:10])
############################################################################
...
ggsave(plot=plot_grid(plotlist=glist1, nrow=2, byrow=FALSE),
       filename=sprintf('%s/Absolute abundance of community structure.pdf',
       dir$figdir),h=21, w=15)
...
ggsave(plot=plot_grid(plotlist=glist1, nrow=2, byrow=FALSE),
       filename=sprintf('%s/Relative abundance of community structure.pdf', dir$figdir),h=21, w=15)
       
#######################################################################################
     
if(FALSE){

	asvname <- unique( do.call(c, lapply(dlist[-7], colnames)))
	
	mat <- matrix(0, ncol=length(unique(asvname)), nrow=5280, 
				  dimnames=list(rownames(sml), unique(asvname)))
	
	for(i in names(dlist)[-7]){ # for test : i=as.character(unique(info$treat1))[1]
	    
	    ## ============================================================ ##
	    ## -- Extracting one treatment matrix
	    tssub <- as.matrix(na.omit(dlist[[i]]))
...
ggsave(plot=g, file=sprintf('%s/NMDS.pdf', dir$figdir),
       w=10, h=15)

...
load.lib(c('ggplot2', 'cowplot', 'RColorBrewer', 'tidyr','gridExtra'))
...
for(i in names(taxa)){ #i=names(simplexs)[1]
	...
	if( all(simp[, c(1,2,4)]== smapT[,c(1,2,3)] & smapT[,c(1,2,3)]==ar[,c(1,2,3)] ) ){
		...
		for(l in 1:nrow(treat)){ #l=1
			...
			summ <- rbind(summ , cbind(sub, asvinfo))
		}
		## ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ##
	...
	if( all(rho[,1:8]==rmse[,1:8])){ rr <- cbind(rho, rmse=rmse[,'rmse']) }
...
pred.fill=c(simplex='royalblue2', smapT='steelblue1', smap0='brown1', AR='orange1', null='darkolivegreen3')
...
for(i in names(simplexpc)){#i= names(simplexpc)[1]
	...
	for(l in 1:ncol(ts)){ #l=2
		...
		lf <- gather( as.data.frame(cbind(inedx=1:880, replicate=c(rep(1,110),rep(2,110),rep(3,110),rep(4,110),rep(5,110),rep(6,110),rep(7,110),rep(8,110)),
				    ...
				    Simplex=simp[, c(l+1)], 'SmapT'=smapT[,c( l+1)],'Smap0'=smap0[,c( l+1)], AR=ar[,c(l+1)], mean=mean[,c(l+1)])), key, 'Predicted value', -c(1:4))
		lf <- lf[lf$tp%in%c(1,4,7,10), ]
		lf$key <- factor(lf$key, levels=c('Simplex', 'SmapT','Smap0', 'AR', 'mean'))
		
		g1 <- ggplot(lf)+
			geom_abline(linetype=2, color='grey50')+
			geom_point(aes(x=Actual, y=`Predicted value`, fill=as.factor(replicate)), shape=23, show.legend=FALSE)+
			facet_grid(key~tp)+
			scale_fill_manual(values=color$replicate)+
			theme_minimal()+
			theme(panel.border =element_rect(fill=NA, size=0.8))+
			labs(title=colnames(tsscale)[l])
		...
		lf22$group.1 <- factor(lf22$group.1, levels=c('simplex', 'smapT','smap0', 'AR', 'null'))
		if(all(is.na(lf22$value))) lf22$value <- 0
		
		g2 <- ggplot(lf22)+
			  geom_boxplot(aes(x=as.factor(tp), y=value, color=group.1, fill=group.1), show.legend=FALSE, alpha=0.7)+
		...
		lf3 <- gather( data.frame(inedx=1:880, replicate=c(rep('replicate 1',110),rep('replicate 2',110),
				     ...
				     simplex=simp[simp[,1]==1, c(l+1)],'smapT'=smapT[smapT[,1]==1,c( l+1)],'smap0'=smap0[smap0[,1]==1,c( l+1)], AR=ar[ar[,1]==1,c(l+1)], mean=mean[mean[,1]==1,c(l+1)]), key, value, -c(1:2))
		g4 <- ggplot(lf3)+
			  ...
			  scale_color_manual(values= c(Actual='grey30',pred.fill))+
			  theme_minimal()+
			  theme(panel.border=element_rect(fill=NA, size=1))+ 
			  labs(x='Time index', y='Actual / Predict value')+
			  facet_wrap(~replicate, nrow=2, scale='free')
		ploltlists[[l]] <- plot_grid(glist1, g4, ncol=1)	  
	}
    ggsave(sprintf("%s/Statics_result_each_asv_%s.pdf", dir$figdir, gsub('/', '_',i)), marrangeGrob(ploltlists, nrow=3, ncol=2),w=21,h=29)
}	

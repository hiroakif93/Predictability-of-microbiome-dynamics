...
load.lib( c('vegan', 'vegetarian', 'ggplot2', 'tidyr', 'cowplot', 'RColorBrewer', 'extrafont','doParallel', 'scales'))

# -- Create directory to save
dir <- make.dir('Fig.1/Diversity')
...
pdf(sprintf('%s/Mean coefficient of variation (ExFig.1-f).pdf', dir$figdir), w=6, h=6)
plot(cvg)
dev.off()
############################################################################
sml$shannon <- NA
for(i in names(dlist)[-7]){ 
    ...
    sml$shannon[sml$treat1==i] <- apply(ts, 1, function(x) d(x, lev='alpha', q=1))
...
ggsave(plot=alpha,
       file=sprintf('%s/Alpha diveristy (ExFig.1-e).pdf', dir$figdir),
       w=6.5, h=6)
...
ggsave(plot=g,
       file=sprintf('%s/Time series of Alpha diveristy (Fig.1-b).pdf', dir$figdir),
       width=10,heigh=13)
...
ggsave(plot=plot_grid(plotlist=gglist, byrow=FALSE),
       filename=sprintf('%s/Bray-Curtis dissimilarity of community structure between time points (Fig.1-a).pdf', dir$figdir),
       width=10,heigh=8)
...
for(i in names(dlist)[-7]){ #i=names(dlist)[1]
	...
	smlsub[smlsub$replicate.id==5,][c(1, seq(10, 110, 10))[-c(4,12)], 1:15]
	
	#par(mfrow=c(1,2))
	#heatmap(as.matrix(na.omit(pathrel[ rownames(smlsub[smlsub$replicate.id==5,]) ,] )), Rowv=NA)
	...
	for(d in as.character(unique(smlsub$time))){ #d=as.character(unique(smlsub$time))[7]
		...
		against <- c(against, colMeans( as.matrix(vegdist(na.omit(pathrel[rownames(smlday), ])), method='bray', na.rm=TRUE)))
	}
...
ggsave(plot=g1, file=sprintf('%s/Bray-Curtis dissimilarity among replicate (Fig.1-c).pdf', dir$figdir), h=8, w=21) 